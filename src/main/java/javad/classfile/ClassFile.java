...
class ClassFileHeader extends dataRead {
    ...
    ClassFileHeader(DataInputStream dStream) {
        ...
        minor_version = (short) readUnsignedShort(dStream);
        major_version = (short) readUnsignedShort(dStream);
...
class classDeclSec extends dataRead implements access_and_modifier_flags {
    ...
    classDeclSec(DataInputStream dStream, ConstPool constPoolSec) {
        ...
        accessFlags = readUnsignedShort(dStream);
        thisClassIx = readUnsignedShort(dStream);
        superClassIx = readUnsignedShort(dStream);

        thisClass = constPoolSec.constPoolElem(thisClassIx);
        superClass = constPoolSec.constPoolElem(superClassIx);

        interfaceCnt = readUnsignedShort(dStream);
    ...
    String getClassName() {
        ...
        if (thisClass != null) {
            if (thisClass instanceof constClass_or_String) {
                name = ObjectNameFormat.toDotSeparator(thisClass.getString());
            }
        }
    ...
    public String getSuperclassName() {
        ...
        return ObjectNameFormat.toDotSeparator(superClassName);
    }
        ...
        if (superClass != null) {
            if (superClass instanceof constClass_or_String) {
                if (superClass.getString().compareTo("java/lang/Object") != 0) {
                    ...
                    superClassName = superClass.getString();
                    superClassName = ObjectNameFormat.toDotSeparator(superClassName);
...
class FieldInfo extends dataRead {
    ...
    FieldInfo(DataInputStream dStream, ConstPool constPoolSec) {
        ...
        access_flags = readUnsignedShort(dStream);
        name_index = readUnsignedShort(dStream);
        desc_index = readUnsignedShort(dStream);
        attr_cnt = readUnsignedShort(dStream);
     ...
     */
        ...
        if (attributes != null) {
            for (Attribute attribute : attributes) {
                if (attribute instanceof constValueAttr) {
                    constAttr = (constValueAttr) attribute;
                    ...
                    isSynthetic = true;
                } else if (attribute instanceof Deprecated) {
                    isDeprecated = true;
                }
            } // for
 ...
 * @author Ian Kaplan
 */
class ClassFieldSection extends dataRead {
    private FieldInfo[] classFields = null;

    ClassFieldSection(DataInputStream dStream, ConstPool constPoolSec) {
        int field_cnt;

        field_cnt = readUnsignedShort(dStream);
        if (field_cnt > 0) {
...
class MethodInfo extends dataRead implements access_and_modifier_flags {
        ...
        if (attr_cnt > 0) {
            ...
            for (int i = 0; i < attr_cnt; i++) {
                ...
                if (attributes[i] instanceof Code)
                    codeAttribute = (Code) attributes[i];
            } // for
        }
    ...
    private String exceptionString() {
        ...
        if (attributes != null) {
            ...
            for (Attribute attribute : attributes) {
                if (attribute instanceof exceptAttr) {
                    exceptAttr eAttr;
                    constClass_or_String[] exceptTab;

                    eAttr = (exceptAttr) attribute;
                    exceptTab = eAttr.getExceptTab();
                    if (exceptTab != null && exceptTab.length > 0) {
                        String className;

                        // build exception list
                        for (constClass_or_String constClass_or_string : exceptTab) {
                            className = constClass_or_string.getString();
                            className = ObjectNameFormat.toDotSeparator(className);
                            if (exceptList == null) {
            ...
            if (methodDesc.charAt(ix) == '(') {
                ...
                for (ix++; ix < len && methodDesc.charAt(ix) != ')'; ix++) {
                    ...
                    if (ch == 'L') { // The type is a class (represented by a class path)
                        ...
                        typeName = ObjectNameFormat.toDotSeparator(objName);
                ...
                if (ix < len && methodDesc.charAt(ix) == ')') {
                    ...
                    if (ch == 'V') // return type is void
                        ...
                        types.returnType = ObjectNameFormat.toDotSeparator(typeName);
                    } else if (typeDesc.isTypeChar(ch)) {
 ...
 * @author Ian Kaplan
 */
class ClassMethodSection extends dataRead {
    MethodInfo[] classMethods;

    ClassMethodSection(DataInputStream dStream,
                       ConstPool constPoolSec,
                       String className) {
        int methodCnt;

        methodCnt = readUnsignedShort(dStream);
        if (methodCnt > 0) {
        ...
        if (classMethods != null) {
            ...
            for (MethodInfo classMethod : classMethods) {
                if (!classMethod.isConstructor()) {
                    methods.add(classMethod.methodString(""));
                }
            }
            return methods;
        } else {
...
class classAttrSec extends dataRead {
    Attribute[] classAttrTab = null;

    classAttrSec(DataInputStream dStream, ConstPool constPoolSec) {

        int numAttr = readUnsignedShort(dStream);

        if (numAttr > 0) {
            classAttrTab = new Attribute[numAttr];
    ...
    String getSrcFileName() {
        ...
        if (classAttrTab != null) {
            for (Attribute Attribute : classAttrTab) {
                if (Attribute instanceof srcFileAttr) {
                    srcFileAttr src;

                    src = (srcFileAttr) Attribute;
...
public class ClassFile {
    ...
    ClassFieldSection classFields = null;
    ClassMethodSection classMethods = null;
    ...
    public ClassFile(DataInputStream dStream) {
        ...
        // u2 fields_count;
        // field_info fields[fields_count];
        classFields = new ClassFieldSection(dStream, classConstPool);

        className = classDecl.getClassName();

        // u2 methods_count;
        // method_info methods[methods_count];
        classMethods = new ClassMethodSection(dStream, classConstPool, className);
